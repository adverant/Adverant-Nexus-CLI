...
import type { CommandResult } from '@nexus-cli/types';
...
export class REPLRenderer {
   ...
   * Render prompt
   */
  getPrompt(namespace?: string, multiline = false): string {
    const prefix = namespace ? `nexus:${namespace}` : 'nexus';
    const indicator = multiline ? '...' : '>';
    return chalk.cyan(prefix) + chalk.gray(` ${indicator} `);
  }
}
