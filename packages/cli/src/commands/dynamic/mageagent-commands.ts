 ...
 *
 * Commands for interacting with the MageAgent service
 */

import type { Command, CommandHandler } from '@adverant/nexus-cli-types';
...
export const orchestrateHandler: CommandHandler = async (args, context) => {
  ...
  if (!service) {
    return {
      success: false,
      error: new Error('MageAgent service not found'),
    };
  }
...
export const analyzeHandler: CommandHandler = async (args, context) => {
  ...
  if (!input) {
    return {
      success: false,
      error: new Error('--input is required'),
    };
  }
...
export const mageagentCommands: Command[] = [
  ...
  },
  {
    ...
    options: [
      {
        long: '--input',
        description: 'Input file or code',
        type: 'string',
        required: true,
      },
      {
