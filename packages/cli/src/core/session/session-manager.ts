...
export class SessionManager implements SessionStorage {
  ...
  async load(nameOrId: string): Promise<Session | null> {
    ...
    try {
      ...
      return {
        ...
        // Handle legacy brainMemories field
        mcpMemories: data.mcpMemories ?? data.brainMemories ?? [],
      };
    } catch (error) {
  ...
  async import(data: string): Promise<Session> {
    ...
    // Handle legacy brainMemories field
    session.mcpMemories = session.mcpMemories ?? session.brainMemories ?? [];
    delete session.brainMemories;
